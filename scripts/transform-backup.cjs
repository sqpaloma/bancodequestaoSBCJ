...
async function main() {
  ...
  if (args.length < 1) {
    console.log(
      'Usage: node scripts/transform-backup.js <backup-folder> [output-folder]',
    );
    console.log('');
    console.log('Example:');
    console.log(
  ...
  const outputFolder =
    args[1] || path.join(path.dirname(backupFolder), 'transformed');
  ...
  console.log('');
  console.log(
    `   npx convex import --prod --append --table themes "${path.join(outputFolder, 'themes.jsonl')}"`,
  );
  console.log(
    `   npx convex import --prod --append --table subthemes "${path.join(outputFolder, 'subthemes.jsonl')}"`,
  );
  console.log(
    `   npx convex import --prod --append --table groups "${path.join(outputFolder, 'groups.jsonl')}"`,
  );
  console.log(
    `   npx convex import --prod --append --table questions "${path.join(outputFolder, 'questions.jsonl')}"`,
  );
  console.log(
    `   npx convex import --prod --append --table questionContent "${path.join(outputFolder, 'questionContent.jsonl')}"`,
  );
  console.log('');
  console.log(
    '3. Run the relink migrations to recreate foreign key references',
  );
}
